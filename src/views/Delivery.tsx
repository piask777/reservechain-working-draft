...
export function Delivery() {
  ...
  return (
    <section className="page-section phase-page">
      ...
      <section className="phase-block"><div className="section-heading"><span className="section-index">WORKSTREAM SPECIFICATIONS</span><h2>Inspect the proposed implementation.</h2><p>Each workstream documents its boundaries, owner decisions and release gates.</p></div><div className="workstream-links"><Link to="/admin"><span>CMS</span><strong>WordPress mapping</strong><small>Content types, API and workflow</small></Link><Link to="/architecture"><span>SYS</span><strong>Architecture</strong><small>Security, operations and audit</small></Link><Link to="/mobile"><span>APP</span><strong>Mobile client</strong><small>Shared API and store gates</small></Link><Link to="/contract-design"><span>SC</span><strong>Contract design</strong><small>Roles, states and mainnet gates</small></Link><Link to="/whitepaper"><span>DOC</span><strong>Whitepaper</strong><small>Outline and localization workflow</small></Link></div></section>
